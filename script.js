...
for(let i=0; i <6 ;i++){
        ...
        document.querySelector(".container").appendChild(box1);
        box1.classList.add(`big_box${i}`);

    for(let j=0; j<5 ;j++){
        let small_box = document.createElement("div");
        document.querySelector(`.big_box${i}`).appendChild(small_box);
        small_box.classList.add(`small_box_prop${i}${j}`);
        small_box.style.cssText = "margin: 2px ; border: 1px solid gray" ; 
    }
}
...
const guess_word = words[randomIndex];
// let guess_word = "table" ;

// random word is generated here 

const evt = document.querySelector(".submit_button");


let itr = 0 ; 

evt.addEventListener("click", ()=>{
    // window.alert("btn is pressed");

    let word = document.querySelector(".user_input");
    let word_guessed = word.value ; 
    // window.alert(word_guessed);

    let word_arr = word_guessed.split("");
    // window.alert(word_arr);
    

    for(let j=0; j<5 ;j++){
        let box = document.querySelector(`.small_box_prop${itr}${j}`);
        
        let ele = document.createElement("p");
        ele.textContent = word_arr[j];
        box.appendChild(ele);
        if(word_arr[j]!==guess_word[j]){
            box.style.cssText="background-color:gray;margin:2px; border: 1px solid gray;";
        }
        if(guess_word.includes(word_arr[j])){
            box.style.cssText="background-color:yellow;margin:2px; border: 1px solid gray;";
        }
        if(word_arr[j]===guess_word[j]){
            box.style.cssText="background-color:green;margin:2px; border: 1px solid gray;";
        }
    ...
    if(itr===6 || word_guessed===guess_word){
        disableElements();
        let message = word_guessed===guess_word ? "Congratulations! You guessed the word!" : `Game over! the word was ${guess_word}` ;
        let msg_box = document.createElement("p");
        msg_box.style.cssText="font-family: Libre Baskerville";

        msg_box.textContent=message;

        document.querySelector(".msg_out").appendChild(msg_box);
    }
